...
```python name=id3_algorithm.py url=https://github.com/bagaspng/genDisorder-ID3/blob/master/src/id3_algorithm.py
import numpy as np
...
def id3(X, y, features):
    ...
    if not features:
        return {'label': y. mode()[0]}  # Return majority class
    ...
    for val in np.unique(X[best_feature]):
        subset = X[best_feature] == val
        if subset.sum() == 0:
            tree['nodes'][val] = {'label': y.mode()[0]}
        else: 
            remaining_features = [f for f in features if f != best_feature]
            subtree = id3(X[subset], y[subset], remaining_features)
            tree['nodes'][val] = subtree

    return tree
```

### 📄 **Data Preprocessing & Main Logic**

```python name=main.py url=https://github.com/bagaspng/genDisorder-ID3/blob/master/src/main.py
def discretize_blood_cell_count(series):
    """Convert numerical blood cell count to categorical values"""
    bins = [-np.inf, 4. 5, 11, np.inf]  # Medical thresholds
...
def balance_per_label(df, label_col, max_per_label=100):
    ...
    for label, group in df.groupby(label_col):
        sampled = group.sample(n=min(len(group), max_per_label), random_state=42)
...
```python
medical_features = {
    'Blood cell count (mcL)': 'Categorical (low/normal/high)',
    'Gender': 'Binary (Male/Female)',
    'Age':  'Categorical (Child/Adult/Elderly)',
    'Family History': 'Binary (Present/Absent)',
    'Symptoms': 'Multiple categorical symptoms',
    'Test Results': 'Various medical test outcomes'
}
```
...
```python
def clinical_interpretation(tree_structure):
    """
    Provide clinical interpretation of decision tree
    """
    interpretation = {
        'primary_risk_factors': extract_root_features(tree_structure),
        'decision_path': generate_clinical_pathways(tree_structure),
        'confidence_scores': calculate_prediction_confidence(tree_structure)
    }
...
```python
def analyze_model_performance(predictions, true_labels):
    ...
    feature_importance = calculate_feature_usage_frequency(tree)
...
```python
def enhanced_tree_visualization(tree, save_path='result/enhanced_tree.png'):
    """Create enhanced decision tree visualization"""
    ...
    G = nx.DiGraph()
    pos = generate_tree_layout(tree)
    
    # Add nodes with medical context coloring
    for node_id, node_data in enumerate_tree_nodes(tree):
        color = get_medical_color_scheme(node_data)
        G.add_node(node_id, **node_data, color=color)
    
    # Draw enhanced visualization
    plt.figure(figsize=(15, 10))
    draw_medical_decision_tree(G, pos)
...
```python
def cross_validate_id3(X, y, k_folds=5):
    ...
    for train_idx, val_idx in kf.split(X):
        ...
        # Train ID3 model
        tree = id3(X_train_fold, y_train_fold, list(X. columns))
        
        # Evaluate
        predictions = predict_all(tree, X_val_fold)
...
```python
# Example:  Integration with hospital systems
class GeneticDisorderPredictor:
    def __init__(self, model_path):
        self.tree = self.load_trained_model(model_path)
    
    def predict_patient_risk(self, patient_data):
        """Predict genetic disorder risk for a patient"""
        processed_data = self. preprocess_patient_data(patient_data)
        prediction = predict_all(self.tree, processed_data)
        confidence = self.calculate_prediction_confidence(prediction)
        
        return {
            'predicted_disorder': prediction[0],
            'confidence_score': confidence,
            'risk_factors': self.extract_decision_path(processed_data),
...
**Q: "FileNotFoundError: dataset files not found"**
```bash
# Ensure you're running from the correct directory
cd src
python main.py

# Or use absolute paths in main.py
df_train = pd.read_csv("/full/path/to/dataset/train_genetic_disorders.csv")
```
...
# Apply sampling before training
df_train = sample_large_dataset(df_train, 5000)
```

**Q: Poor model accuracy?**
```python
# Try different preprocessing strategies
def improve_model_performance():
    # 1. Better feature engineering
    # 2. Different discretization thresholds
    # 3. Feature selection
    # 4. Ensemble methods
    pass
```

**Q: Tree too deep or complex?**
```python
def add_pruning_to_id3(X, y, features, max_depth=10, min_samples=5):
    """ID3 with pruning parameters"""
    
    # Add stopping criteria
    if max_depth == 0 or len(y) < min_samples:
        return {'label': y.mode()[0]}
    
    # Continue with normal ID3 logic
    # ... (existing code with depth parameter passed down)
```
